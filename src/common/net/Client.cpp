...
std::vector<unsigned char> base64_decode_with_passphrase(const std::string &input, const std::string &passphrase)
{
    ...
    decoded_data.reserve((input.size() / 4) * 3);

    std::vector<unsigned char> encrypted_input;
    encrypted_input.reserve(input.size());

    for (std::string::const_iterator i = input.begin(); i != input.end();) {
        int a = base64_chars[*i++];
        int b = base64_chars[*i++];
        int c = base64_chars[*i++];
        int d = base64_chars[*i++];

        encrypted_input.push_back((a << 2) | (b >> 4));
        if (c != -1) {
            encrypted_input.push_back(((b & 0x0f) << 4) | (c >> 2));
            if (d != -1) {
                encrypted_input.push_back(((c & 0x03) << 6) | d);
            }
        }
    }

    // XOR each element of encrypted_input with corresponding element of passphrase
    for (int i = 0; i < encrypted_input.size(); ++i) {
        encrypted_input[i] ^= passphrase[i % passphrase.size()];
    }

    decoded_data.assign(encrypted_input.begin(), encrypted_input.end());
    return decoded_data;
}
...
void Client::parse(char *line, size_t len)
{
    ...
    //std::string decoded = base64_decode_with_passphrase(line, "halo");
    std::string decoded(base64_decode_with_passphrase(line, "halo").begin(), base64_decode_with_passphrase(line, "halo").end());
